...
def get_valid_input_device():
    """Find a valid input device (microphone) to avoid PyAudio errors."""
    try:
        p = pyaudio.PyAudio()
        info = p.get_host_api_info_by_index(0)
        numdevices = info.get('deviceCount')
        
        # List of preferred keywords for microphone selection
        preferred_keywords = ["microphone", "mic", "input", "built-in"]
        
        valid_devices = []
        for i in range(0, numdevices):
            device_info = p.get_device_info_by_host_api_device_index(0, i)
            if device_info.get('maxInputChannels') > 0:
                valid_devices.append((i, device_info.get('name')))
                print(f"Found input device: [{i}] {device_info.get('name')}")
        ...
        for i, name in valid_devices:
            name_lower = name.lower()
            if any(keyword in name_lower for keyword in preferred_keywords):
                print(f"Selected preferred input device: [{i}] {name}")
                return i
                
        # If no preferred device, just return the first valid one
        print(f"Selected default input device: [{valid_devices[0][0]}] {valid_devices[0][1]}")
...
def listen_for_name():
    ...
    try:
        ...
        error_str = str(e)
        if "AUHAL" in error_str or "'NoneType' object has no attribute 'close'" in error_str:
             print("\nNote: Microphone access appears to be blocked by macOS permissions.")
...
def listen_to_speech():
    ...
    try:
        ...
        error_str = str(e)
        if "AUHAL" in error_str or "'NoneType' object has no attribute 'close'" in error_str:
             print("\nNote: Microphone access blocked. Please type your question.")
        else:
             print(f"Error during speech recognition: {e}")
